...
fn build_report(matrix: &StateMatrix) -> KeyboardReport {
    ...
    let kcs_2: [Keycode; 6] = [KC_O, KC_E, KC_I, KC_U, KC_1, KC_2];
    let mut count = 0;
    for col in kcs_2 {
        if matrix[2][count] {
            push_key(col);
    ...
    let kcs_5: [Keycode; 6] = [KC_0, KC_GRAVE, KC_MINUS, KC_EQUAL, KC_M, KC_R];
    let mut count = 0;
    for col in kcs_5 {
        if matrix[5][count] {
            push_key(col);
    ...
    let kcs_7: [Keycode; 6] = [KC_RIGHT_BRACKET, KC_SPACE, KC_G, KC_T, KC_K, KC_S];
    let mut count = 0;
    for col in kcs_7 {
        if matrix[7][count] {
            push_key(col);
    ...
    let kcs_9: [Keycode; 6] = [KC_B, KC_H, KC_J, KC_L, KC_SLASH, KC_BACKSLASH];
    let mut count = 0;
    for col in kcs_9 {
        if matrix[9][count] {
            push_key(col);
